...
async def Pharmacokinetics_predict(
    ...
    try:
        async with httpx.AsyncClient(timeout=DEFAULT_TIMEOUT) as client:
            url = "http://101.126.67.113:8004/sync_mcp_pk_predict"
            response = await client.post(
                url,
                json={
            ...
            if not res.data.pk_result.data:
                return RES(
                    code=-1,
                    msg=f"Pharmacokinetics predict error: {res.data.pk_result.msg}",
                )
...
async def ToxScan_predict(
    ...
    try:
        async with httpx.AsyncClient(timeout=DEFAULT_TIMEOUT) as client:
            ...
            payload = {"type": "smi", "content": [data.smiles]}
            response = await client.post(url, json=payload)
            response.raise_for_status()
            res_raw = response.json()
            res = RES[ToxResult](
                code=res_raw["code"], msg=res_raw["msg"], data=res_raw["data"][0]
            )
